    ...
    */
    ...
    onDMSPreviewFile(node) {
        const attachmentData = {
            ...
            displayName: node.data.data.name,
            defaultSource: `/web/content?id=${node.data.data.id}&field=content&model=dms.file&filename_field=name`,
        };
        
        try {
            const AttachmentModel = this.store.Attachment || (this.store.models && this.store.models.Attachment);
            if (AttachmentModel && typeof AttachmentModel.insert === 'function') {
                ...
                if (this.fileViewer && typeof this.fileViewer.open === 'function') {
                    this.fileViewer.open(attachmentData);
                } else {
                    window.open(attachmentData.defaultSource, '_blank');
                }
            }
        } catch (e) {
            console.error("DMS Preview Error", e);
            window.open(attachmentData.defaultSource, '_blank');
        }
    }
    get showDragZone() {
